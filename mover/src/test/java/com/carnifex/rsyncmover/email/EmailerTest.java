...
public class EmailerTest {
    ...
    public void nextEmailTime_inOneMinute() throws Exception {
        final Emailer emailer = new Emailer(false, null, null, LocalTime.now().plusMinutes(1));
    ...
    public void nextEmailTime_now() throws Exception {
        final Emailer emailer = new Emailer(false, null, null, LocalTime.now());
        final long next = emailer.nextEmailTime() / 1000L / 60L;
        Assert.assertTrue(next > 0);
    }
    ...
    public void nextEmailTime_oneMinuteAgo() throws Exception {
        final Emailer emailer = new Emailer(false, null, null, LocalTime.now().minusMinutes(1));
        final long next = emailer.nextEmailTime() / 1000L / 60L;
        Assert.assertTrue(next > 0);
    }
    ...
    public void nextEmailTime_inAnHour() throws Exception {
        final Emailer emailer = new Emailer(false, null, null, LocalTime.now().plusHours(1));
        final long next = emailer.nextEmailTime() / 1000L / 60L;
        Assert.assertTrue(next > 0);
    }
}